(() => {
  // pages/test-javascript-vs-audio-frame-time/src/index.js
  ...
  var previousJSTimeMillis = null;
  var previousAudioTimeMillis = null;
  var buffer = [];
  var bufferMaxSize = 700;
  function draw(currentJSTimeMillis) {
    let currentAudioTimeMillis = audioContext.currentTime * 1e3;
    ...
    for (let i = 0; i < buffer.length; i++) {
      ...
      let y2 = mapLinear(minDelta, buffer[i], maxDelta, canvas.height, 0);
      if (i == 0) {
        ctx.moveTo(x, y2);
      } else {
        ctx.lineTo(x, y2);
