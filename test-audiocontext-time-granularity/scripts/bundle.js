(() => {
  // pages/test-audiocontext-time-granularity/src/index.js
  ...
  channel.port1.onmessage = () => {
    ...
    if (samples.length < samplesMaxSize && performance.now() - startTime < 2e3) {
  ...
  function calculateDeltas() {
    ...
    for (let i = 1; i < samples.length; i++) {
      buffer.push(1e3 * (samples[i] - previousSample));
